...
def bank_report():
    with gobal.con:
        if not session.get("name"):
            ...
            bank_code=''
            return render_template('/report/transfer/home.html', dollar=dollar, from_date=timestampStr, to_date=timestampStr,bank_from=bank_from,bank_code=bank_code)
...
def bank_report_sch():
    with gobal.con:
        if not session.get("name"):
            ...
            curs.execute(sql)
            bank_from = curs.fetchall()
            return render_template('/report/transfer/home.html', kip=kip, from_date=from_date, to_date=to_date,bank_from=bank_from,bank_code=bank_code)