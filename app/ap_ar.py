...
def ap():
    with gobal.con:
        if not session.get("name"):
            ...
            curb.execute(sql_p)
            prov_list = curb.fetchall()
            return render_template('ap & ar/ap.html', rate_trans = rate_trans, prov_list = prov_list)
...
def ap_pay():
    with gobal.con:
        if not session.get("name"):
            ...
            return render_template('ap & ar/ap_pay.html', rate_trans = rate_trans)
...
def send_apid(id):
    with gobal.con:
        if not session.get("name"):
            ...
            gobal.con.commit()
            return render_template('ap & ar/set_ap_copy.html',selectapid=selectapid,rate_trans = rate_trans, curent=curent)
...
def send_arid(id):
    with gobal.con:
        if not session.get("name"):
            ...
            gobal.con.commit()
        return render_template('ap & ar/set_ar_copy.html',selectarid=selectarid,rate_trans = rate_trans, curent=curent)
